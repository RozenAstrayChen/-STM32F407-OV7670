...
void USART1_Conf(void)
{
	...
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_USART2, ENABLE);                        //使能USART1的APB2时钟
	 /*配置复用管脚TXD(PA9)和RXD(PA10)连接到USART1*/
	GPIO_PinAFConfig(GPIOA,GPIO_PinSource2,GPIO_AF_USART2);						  //配置PA9复用连接到USART1
    GPIO_PinAFConfig(GPIOA,GPIO_PinSource3,GPIO_AF_USART2);				      //配置PA10复用连接到USART1

	/*配置USART1的发送管脚TXD(PA9)为复用推挽输出*/
	GPIO_InitStructure.GPIO_Pin=GPIO_Pin_3;                                       //设置初始化GPIO为PIN9
	...
	GPIO_InitStructure.GPIO_Pin=GPIO_Pin_2;                                      //设置初始化GPIO为PIN10
